(() => {
  ...
  const state = Object.assign({ coins:250, wood:0, bananas:0, stone:0, xp:0, level:1, totalEarned:0, quest:0, sound:true, boostUntil:0, trucks:{} }, JSON.parse(localStorage.getItem('idle-island-save') || '{}'));
  ...
  function drawVehicles(){
    const u=(t*.035)%1; let x=300+(1040-300)*u,y=240+(440-240)*u;ctx.save();ctx.translate(x,y);ctx.rotate(.22);ctx.fillStyle='#e94f47';ctx.strokeStyle='#2c4950';ctx.lineWidth=2;ctx.fillRect(-14,-8,28,16);ctx.strokeRect(-14,-8,28,16);ctx.fillStyle='#d9eff3';ctx.fillRect(-8,-6,8,12);ctx.restore();
  ...
  function update(dt){
    t+=dt; const boost=Date.now()<state.boostUntil?2:1; const income=sites.filter(s=>s.built).reduce((n,s)=>n+s.base*s.level*(state.trucks[s.id]||1),0)*boost; state.coins+=income*dt/10; state.totalEarned+=income*dt/10;
  ...
  function updateHUD(){
    ...
    const remain=Math.max(0,Math.ceil((state.boostUntil-Date.now())/1000));$('boostTime').textContent=remain?`${String(Math.floor(remain/60)).padStart(2,'0')}:${String(remain%60).padStart(2,'0')}`:'04:00';$('speedBoost').classList.toggle('active',remain>0);
  ...
  canvas.addEventListener('pointerup',(e)=>{if(!$('welcome').classList.contains('closed'))return;const p=screenToWorld(e.clientX,e.clientY);const s=sites.find(x=>Math.hypot(p.x-x.x,p.y-(x.y-55))<55);if(!s)return;if(!s.built){focused=s;openBuild()}else{collect(s,e.clientX,e.clientY);openFacility(s)}});
  function collect(s,x,y){const gain=Math.max(10,s.base*s.level*3);state.coins+=gain;state.xp+=4; if(s.id==='banana')state.bananas+=s.level;if(s.id==='lumber')state.wood+=s.level;if(state.xp>=state.level*100){state.xp-=state.level*100;state.level++;toast(`섬 레벨 ${state.level} 달성!`)}popIncome(x,y,`+${gain}`);save()}
  ...
  function openFacility(s){
    focused=s;$('sheet').classList.add('hidden');$('facilityIcon').textContent=s.icon;$('facilityName').textContent=s.name;$('facilityLevel').textContent=`LV.${s.level}`;$('facilityStars').textContent='★'.repeat(Math.min(5,Math.floor(s.level/2)))+'☆'.repeat(Math.max(0,5-Math.floor(s.level/2)));$('facilityIncome').textContent=fmt(s.base*s.level*(state.trucks[s.id]||1));$('facilityTime').textContent=Math.max(4,16-s.level*.8).toFixed(1)+'초';const cost=Math.round(s.cost*Math.pow(1.55,s.level));$('facilityUpgrade').textContent=`● ${fmt(cost)}`;$('facilityUpgrade').disabled=state.coins<cost;const trucks=state.trucks[s.id]||1;$('truckCount').textContent=`${trucks}/3`;$('truckUpgrade').textContent=`● ${fmt(900*trucks*trucks)}`;$('truckUpgrade').disabled=trucks>=3;$('perfectChance').textContent=Math.min(50,5+s.level*2)+'%';$('facilityPanel').classList.remove('hidden');
  ...
  $('mainBuildButton').onclick=openBuild;$('closeSheet').onclick=()=>$('sheet').classList.add('hidden');$('startButton').onclick=()=>{$('welcome').classList.add('closed');toast('반짝이는 + 버튼을 눌러 건설하세요!')};
  $('closeFacility').onclick=()=>$('facilityPanel').classList.add('hidden');$('speedBoost').onclick=()=>{state.boostUntil=Date.now()+240000;toast('4분 동안 생산 속도 2배!');save()};
