(() => {
  ...
  let scale=1, offsetX=0, offsetY=0, t=0, last=performance.now(), saveTimer=0, focused=null, drag=null;
  ...
  function openGuide(){openSheet('가이드북','MAYOR HANDBOOK',`<article class="guide-section"><span class="guide-number">1</span><h3>작은 섬에서 시작</h3><p>PC는 마우스 왼쪽 버튼을 누른 채 드래그하고, 모바일은 화면을 손가락으로 밀어 지도를 이동합니다. 시설 아이콘을 짧게 눌러 수익을 모으세요.</p></article><article class="guide-section"><span class="guide-number">2</span><h3>15레벨 마일스톤</h3><p>시설이 15레벨 오를 때마다 트럭 외형이 진화하고, 생산 수익과 속도가 각각 50%씩 강화되며 완벽 배송 확률도 1% 증가합니다. 완벽 배송 수익은 2배입니다.</p></article><article class="guide-section"><span class="guide-number">3</span><h3>도로 업그레이드</h3><p>오른쪽의 ROAD 버튼에서 코인을 사용하면 모든 차량 속도가 레벨당 22% 빨라집니다. 최고 레벨은 10입니다.</p></article><article class="guide-section"><span class="guide-number">4</span><h3>10분 부스트</h3><p>게임을 시작하면 10분 동안 생산 수익이 2배가 됩니다. 타이머가 끝난 뒤 부스트 버튼으로 다시 시작할 수 있습니다.</p></article><article class="guide-section"><span class="guide-number">5</span><h3>잠금과 저장</h3><p>어두운 그늘 지역은 표시된 섬 레벨에 도달하면 열립니다. 진행 상황은 이 브라우저에 자동 저장됩니다.</p></article>`)}
