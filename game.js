(() => {
  ...
  const state = Object.assign({ coins:250, wood:0, bananas:0, stone:0, xp:0, level:1, levelEarnings:0, totalEarned:0, quest:0, sound:true, boostUntil:0, trucks:{}, roadLevel:1, craneLevel:1, truckLoadLevel:1, deliveredCargo:0, cargoQueue:0, cargoProgress:0 }, JSON.parse(localStorage.getItem('idle-island-save') || '{}'));
  if (state.sites) sites.forEach((s,i) => Object.assign(s,state.sites[i] || {}));
  let scale=1, offsetX=0, offsetY=0, t=0, last=performance.now(), saveTimer=0, focused=null, drag=null, lastShipmentCycle=-1, lastTruckCycle=0;
  ...
  function drawVehicles(){
    ...
    const starterRoute=[[300,470],[335,575],[465,650],[625,630],[760,540],[730,470],[610,440],[450,440]];
    const mainRoute=[[0,190],[180,190],[300,240],[425,260],[560,350],[730,365],[875,425],[1040,440],[1280,365],[1040,440],[875,425],[730,365],[560,350],[425,260],[300,240],[180,190]];
    const route=state.level<3?starterRoute:mainRoute;
    ...
    for(let i=0;i<count;i++){
      const u=(baseProgress+i/count)%1;
      const point=routePoint(route,u),next=routePoint(route,(u+.01)%1);
      const hasCargo=(state.cargoQueue||0)>i&&u>.32;
      const producers=sites.filter(s=>s.built&&s.id!=='port'),producer=producers[i%Math.max(1,producers.length)];
      const cargoColors={banana:'#e4c738',lumber:'#9f642d',mine:'#8b9598'};
      drawTopDownTruck(point.x,point.y,Math.atan2(next.y-point.y,next.x-point.x),tier,colors,lengths,hasCargo,cargoColors[producer?.id]||'#d88b2d');
    }
  ...
  function activeTruckCount(){const fleet=availableTruckCount(),waiting=Math.ceil(state.cargoQueue||0);return fleet?Math.min(fleet,Math.max(1,waiting)):0}
  ...
  function drawCargoShip(){
    ...
    const loadedCount=Math.min(3,Math.floor(loadProgress*3));
    const slot=Math.min(2,Math.floor(loadProgress*3));
    const move=(loadProgress*3)%1;
  ...
  function update(dt){
    t+=dt; const boost=Date.now()<state.boostUntil?2:1; const incomePerSecond=sites.filter(s=>s.built).reduce((n,s)=>n+facilityIncome(s)/facilityTime(s),0)*boost,passiveIncome=incomePerSecond*dt;state.coins+=passiveIncome;recordEarnings(passiveIncome);
    const producers=sites.filter(s=>s.built&&s.id!=='port');state.cargoProgress=(state.cargoProgress||0)+producers.reduce((n,s)=>n+dt/(facilityTime(s)+truckLoadingTime()),0);if(state.cargoProgress>=1){const made=Math.floor(state.cargoProgress);state.cargoQueue=Math.min(50,(state.cargoQueue||0)+made);state.cargoProgress-=made}
    const truckCycle=Math.floor(t*.055*(1+(state.roadLevel-1)*.22));if(truckCycle>lastTruckCycle){lastTruckCycle=truckCycle;const delivered=Math.min(Math.floor(state.cargoQueue||0),activeTruckCount());if(delivered>0){state.cargoQueue-=delivered;state.deliveredCargo=Math.min(12,(state.deliveredCargo||0)+delivered);toast(`트럭 ${delivered}대 하역 완료 · 화물 ${delivered}개`)}}
    const shipDuration=shipCycleDuration(),shipmentCycle=Math.floor(t/shipDuration),departAt=6+craneLoadingTime();if(t%shipDuration>=departAt&&shipmentCycle>lastShipmentCycle){lastShipmentCycle=shipmentCycle;const port=sites.find(s=>s.id==='port');if(port?.built){const payout=Math.max(100,Math.round(sites.filter(s=>s.built).reduce((n,s)=>n+facilityIncome(s),0)*2));state.coins+=payout;recordEarnings(payout);state.deliveredCargo=0;toast(`화물선 출항 · 운송 수익 +${fmt(payout)}`)}}
  ...
  function collect(s,x,y){let gain=Math.max(10,Math.round(facilityIncome(s)*3));const perfect=Math.random()*100<perfectChance(s);if(perfect)gain*=2;state.coins+=gain;recordEarnings(gain);if(s.id==='banana')state.bananas+=s.level;if(s.id==='lumber')state.wood+=s.level;if(perfect)toast(`완벽 배송! 수익 2배 +${gain}`);popIncome(x,y,perfect?`PERFECT! +${gain}`:`+${gain}`);save()}
  ...
  function openTrade(){const income=Math.round(sites.filter(s=>s.built).reduce((n,s)=>n+facilityIncome(s)/facilityTime(s)*10,0)),craneCost=craneUpgradeCost(),truckCost=truckLoadUpgradeCost();openSheet('해외 무역','OVERSEAS TRADE',`<div class="wide-card"><span>⌁</span><div><h3>화물선</h3><p>크레인 선적 후 자동 출항 · 하역 화물 ${state.deliveredCargo||0}개</p></div><b>+${income}</b></div><div class="wide-card"><span>▣</span><div><h3>트럭 상차 Lv.${state.truckLoadLevel}</h3><p>물건 1개 상차 ${truckLoadingTime().toFixed(1)}초 · 레벨마다 15% 단축</p></div><button id="truckLoadUpgrade" ${state.truckLoadLevel>=10?'disabled':''}>${state.truckLoadLevel>=10?'MAX':`● ${fmt(truckCost)}`}</button></div><div class="wide-card"><span>✣</span><div><h3>크레인 Lv.${state.craneLevel}</h3><p>선박 선적 ${craneLoadingTime().toFixed(1)}초 · 레벨마다 15% 단축</p></div><button id="craneUpgrade" ${state.craneLevel>=10?'disabled':''}>${state.craneLevel>=10?'MAX':`● ${fmt(craneCost)}`}</button></div><div class="wide-card"><span>▦</span><div><h3>즉시 수출</h3><p>바나나와 목재를 코인으로 교환합니다.</p></div><button id="sellGoods">SELL</button></div>`);$('truckLoadUpgrade').onclick=upgradeTruckLoading;$('craneUpgrade').onclick=upgradeCrane;$('sellGoods').onclick=()=>{const gain=state.bananas*18+state.wood*24;if(!gain)return toast('수출할 상품이 없습니다');state.coins+=gain;recordEarnings(gain);state.bananas=0;state.wood=0;toast(`상품 판매 +${gain}`);save();openTrade()}}
  ...
  function openGuide(){openSheet('가이드북','MAYOR HANDBOOK',`<article class="guide-section"><span class="guide-number">1</span><h3>작은 섬과 지도</h3><p>모든 시설과 운송수단은 위에서 내려다보는 탑다운 시점입니다. 좌클릭 또는 터치 드래그로 지도를 이동합니다.</p></article><article class="guide-section"><span class="guide-number">2</span><h3>생산자와 트럭</h3><p>생산 시설 15레벨마다 해당 화물 전용 트럭이 1대 추가됩니다. 대기 물량이 늘면 트럭이 추가 출발하며 한 대는 물건 하나만 운송합니다.</p></article><article class="guide-section"><span class="guide-number">3</span><h3>하역과 선박</h3><p>트럭은 도로로만 이동해 하역장에 상자를 놓습니다. 크레인은 상자를 배에 선적하며 트럭 상차와 크레인 선적 시간은 무역 메뉴에서 따로 업그레이드합니다.</p></article><article class="guide-section"><span class="guide-number">4</span><h3>15레벨 보너스</h3><p>시설 15레벨마다 수익과 속도가 50% 강화되고 완벽 배송 확률이 1% 증가합니다. 완벽 배송 가격은 2배입니다.</p></article><article class="guide-section"><span class="guide-number">5</span><h3>섬 레벨</h3><p>수익 목표를 달성하면 섬 레벨이 오릅니다. 다음 목표 금액은 이전 목표의 3배이며, 레벨에 따라 그늘 지역이 해금됩니다.</p></article>`)}
