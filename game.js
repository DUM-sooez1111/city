(() => {
  ...
  const state = Object.assign({ coins:250, wood:0, bananas:0, stone:0, xp:0, level:1, levelEarnings:0, totalEarned:0, quest:0, sound:true, boostUntil:0, trucks:{}, roadLevel:1, craneLevel:1, truckLoadLevel:1, deliveredCargo:0, deliveredValue:0, cargoQueue:0, cargoValueQueue:0, cargoProgress:0 }, JSON.parse(localStorage.getItem('idle-island-save') || '{}'));
  if (state.sites) sites.forEach((s,i) => Object.assign(s,state.sites[i] || {}));
  let scale=1, offsetX=0, offsetY=0, t=0, last=performance.now(), saveTimer=0, focused=null, drag=null, lastShipmentCycle=-1, lastTruckCycle=0, lastShipCargo=0;
  ...
  function update(dt){
    ...
    const truckCycle=Math.floor(t*.055*(1+(state.roadLevel-1)*.22));if(truckCycle>lastTruckCycle){lastTruckCycle=truckCycle;const delivered=Math.min(Math.floor(state.cargoQueue||0),activeTruckCount());if(delivered>0){const averageValue=(state.cargoValueQueue||0)/Math.max(1,state.cargoQueue||0),movedValue=averageValue*delivered;state.cargoQueue-=delivered;state.cargoValueQueue=Math.max(0,state.cargoValueQueue-movedValue);state.deliveredCargo=(state.deliveredCargo||0)+delivered;state.deliveredValue=(state.deliveredValue||0)+movedValue;toast(`트럭 ${delivered}대 하역 완료 · 화물 ${delivered}개`)}}
  ...
  function openGuide(){openSheet('가이드북','MAYOR HANDBOOK',`<article class="guide-section"><span class="guide-number">1</span><h3>작은 섬과 지도</h3><p>모든 시설과 운송수단은 위에서 내려다보는 탑다운 시점입니다. 좌클릭 또는 터치 드래그로 지도를 이동합니다.</p></article><article class="guide-section"><span class="guide-number">2</span><h3>생산자와 트럭</h3><p>생산 시설 15레벨마다 해당 화물 전용 트럭이 1대 추가됩니다. 대기 물량이 늘면 트럭이 추가 출발하며 한 대는 물건 하나만 운송합니다.</p></article><article class="guide-section"><span class="guide-number">3</span><h3>하역·선적·정산</h3><p>트럭은 도로로만 이동해 하역장에 상자를 놓고 크레인이 배에 선적합니다. 현금은 화물을 실은 배가 출항할 때만 들어오며 빈 배의 수익은 0입니다.</p></article><article class="guide-section"><span class="guide-number">4</span><h3>운송 업그레이드</h3><p>트럭 상차와 크레인 선적 시간을 무역 메뉴에서 따로 줄일 수 있습니다. 시설 15레벨마다 수익·속도 +50%, 완벽 확률 +1%가 적용됩니다.</p></article><article class="guide-section"><span class="guide-number">5</span><h3>섬 레벨</h3><p>선박 출항 수익 목표를 달성하면 섬 레벨이 오릅니다. 다음 목표 금액은 이전 목표의 3배이며, 레벨에 따라 그늘 지역이 해금됩니다.</p></article>`)}
