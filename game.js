(() => {
  ...
  function openGuide(){openSheet('가이드북','MAYOR HANDBOOK',`<article class="guide-section"><span class="guide-number">1</span><h3>작은 섬과 지도</h3><p>모든 시설과 운송수단은 위에서 내려다보는 탑다운 시점입니다. 좌클릭 또는 터치 드래그로 지도를 이동합니다.</p></article><article class="guide-section"><span class="guide-number">2</span><h3>생산자와 트럭</h3><p>트럭은 생산자 앞까지 빈 차로 이동해 상자 하나를 받은 뒤 하역장으로 돌아옵니다. 생산 시설 15레벨마다 전용 트럭이 1대 추가되며, 배차된 트럭은 사라지지 않습니다.</p></article><article class="guide-section"><span class="guide-number">3</span><h3>하역·선적·정산</h3><p>트럭 한 대는 화물 하나를 하역장에 놓고 크레인이 배에 선적합니다. 현금은 화물을 실은 배가 출항할 때만 들어오며 빈 배의 수익은 0입니다.</p></article><article class="guide-section"><span class="guide-number">4</span><h3>운송 업그레이드</h3><p>시설 15레벨마다 해당 생산자의 트럭만 새 외형으로 바뀌고 차량 속도가 25% 증가합니다. 같은 외형 단계의 트럭은 생산자가 달라도 같은 속도이며, 도로 업그레이드는 모든 차량에 적용됩니다.</p></article><article class="guide-section"><span class="guide-number">5</span><h3>섬 레벨</h3><p>선박 출항 수익 목표를 달성하면 섬 레벨이 오릅니다. 다음 목표 금액은 이전 목표의 3배이며, 레벨에 따라 그늘 지역이 해금됩니다.</p></article>`)}
