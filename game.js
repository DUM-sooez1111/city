(() => {
  ...
  const sites = [
    ...
    { id:'coffee', name:'커피 농장', icon:'☕', x:-330, y:430, cost:1600, base:85, color:'#a96b3f', built:false, level:0 },
    { id:'salt', name:'천일염 염전', icon:'🧂', x:360, y:115, markerX:275, markerY:95, unlock:2, route:'salt', cost:260, base:20, color:'#d9e6df', extra:true, built:false, level:0 },
    { id:'sugar', name:'사탕수수 농장', icon:'🌾', x:1080, y:335, markerX:1115, markerY:405, unlock:3, route:'sugar', cost:520, base:38, color:'#94c95d', extra:true, built:false, level:0 },
    { id:'coconut', name:'코코넛 농장', icon:'🥥', x:-485, y:185, markerX:-510, markerY:95, unlock:4, route:'coconut', cost:780, base:48, color:'#7fb84c', extra:true, built:false, level:0 },
    { id:'flowers', name:'화훼 농장', icon:'🌺', x:-395, y:250, markerX:-300, markerY:265, unlock:5, route:'flowers', cost:1150, base:64, color:'#e277a5', extra:true, built:false, level:0 },
    { id:'cocoa', name:'카카오 농장', icon:'🍫', x:-450, y:510, markerX:-515, markerY:565, unlock:6, route:'cocoa', cost:2100, base:110, color:'#81502f', extra:true, built:false, level:0 },
    { id:'steel', name:'제철소', icon:'⚙️', x:1170, y:285, markerX:1230, markerY:260, unlock:7, route:'steel', cost:3200, base:155, color:'#66777d', extra:true, built:false, level:0 },
    { id:'fish', name:'수산 양식장', icon:'🐟', x:1120, y:555, markerX:1200, markerY:555, unlock:7, route:'fish', cost:3900, base:185, color:'#4c9fca', extra:true, built:false, level:0 },
    { id:'tea', name:'차 농장', icon:'🍵', x:730, y:-315, markerX:690, markerY:-405, unlock:8, route:'tea', cost:5200, base:240, color:'#55964d', extra:true, built:false, level:0 },
    { id:'honey', name:'양봉장', icon:'🍯', x:930, y:-220, markerX:850, markerY:-175, unlock:9, route:'honey', cost:7600, base:320, color:'#dfa82e', extra:true, built:false, level:0 },
    { id:'dairy', name:'낙농 목장', icon:'🥛', x:1050, y:-300, markerX:1135, markerY:-365, unlock:10, route:'dairy', cost:11000, base:430, color:'#f1eee1', extra:true, built:false, level:0 },
  ];
  ...
  function resize(){
    ...
    scale=Math.max(r.width/WORLD.w,r.height/(r.width<r.height?620:WORLD.h)); const cameraX=r.width<r.height&&state.level<3?350:WORLD.minX+WORLD.w/2; offsetX=r.width/2-cameraX*scale; offsetY=r.height/2-360*scale;clampCamera();
  ...
  function drawIsland(){
    ...
    road([[820,0],[860,-72],[905,-135],[930,-220],[1050,-300]],34);
    road([[930,-220],[820,-305],[730,-315]],24);
  ...
  function drawVehicles(){
    ...
    for(let i=0;i<count;i++){
      ...
      const cargoSite=sites.find(s=>s.id===state.truckLoads[i]?.producerId);
      drawTopDownTruck(point.x,point.y,Math.atan2(next.y-point.y,next.x-point.x),tier,colors,lengths,Boolean(state.truckLoads[i]),cargoSite?.color||producer?.color||'#d88b2d');
    }
  ...
  function drawNewIslandFog(){if(state.level>=6)return;ctx.save();ctx.beginPath();ctx.moveTo(-565,340);ctx.bezierCurveTo(-505,278,-380,268,-255,292);ctx.bezierCurveTo(-145,312,-98,372,-112,446);ctx.bezierCurveTo(-82,525,-155,635,-295,680);ctx.bezierCurveTo(-470,705,-585,615,-590,475);ctx.bezierCurveTo(-592,415,-585,370,-565,340);ctx.closePath();ctx.clip();ctx.fillStyle='rgba(20,39,36,.78)';ctx.fillRect(-600,265,520,440);ctx.globalAlpha=.22;for(let i=0;i<7;i++){ctx.fillStyle='#071d20';ctx.beginPath();ctx.arc(-535+i*68,370+(i%3)*92,48+(i%2)*16,0,7);ctx.fill()}ctx.globalAlpha=1;ctx.fillStyle='white';ctx.textAlign='center';ctx.font='900 38px Nunito';ctx.fillText('🔒',-330,430);ctx.font='900 18px Nunito';ctx.fillText('LEVEL 6',-330,465);ctx.font='900 12px Nunito';ctx.fillText('새로운 섬 · 커피 농장',-330,487);ctx.restore()}
  function draw(){ctx.setTransform(DPR*scale,0,0,DPR*scale,DPR*offsetX,DPR*offsetY);drawWater();drawIsland();drawScenery();sites.forEach(drawSite);drawVehicles();drawLockedFog();drawNewIslandFog();}
  function update(dt){
  ...
  function unlockLevel(s){return s.unlock||({banana:1,lumber:2,mine:3,port:5,coffee:6}[s.id]||1)}
  ...
  function openMap(){openSheet('새로운 섬','ISLAND EXPLORATION',`<div class="wide-card"><span>🏝️</span><div><h3>Emerald Bay</h3><p>현재 개발 중인 무역의 섬</p></div><b>NOW</b></div><div class="wide-card"><span>🌲</span><div><h3>Cedar Isle</h3><p>북쪽 대각선 도로로 이어지는 숲의 섬</p></div><b>OPEN</b></div><div class="wide-card"><span>🌴</span><div><h3>Palm Key</h3><p>서쪽 상단 도로로 바로 이어지는 휴양 섬</p></div><b>OPEN</b></div><div class="wide-card" style="${state.level<6?'filter:grayscale(1);opacity:.65':''}"><span>☕</span><div><h3>Coffee Key</h3><p>${state.level<6?'섬 레벨 6에 왼쪽 다리와 커피 농장 개방':'다리를 건너 커피 농장을 건설할 수 있습니다'}</p></div><b>${state.level<6?'🔒':'OPEN'}</b></div><div class="wide-card" style="${state.level<8?'filter:grayscale(1);opacity:.65':''}"><span>✈️</span><div><h3>Island Airport</h3><p>${state.level<8?'섬 레벨 8에 비행기 운항 개방':'비행기가 착륙·활주·이륙을 반복합니다'}</p></div><b>${state.level<8?'🔒':'OPEN'}</b></div>`)}
  function openGuide(){openSheet('가이드북','MAYOR HANDBOOK',`<article class="guide-section"><span class="guide-number">1</span><h3>작은 섬과 지도</h3><p>모든 시설과 운송수단은 위에서 내려다보는 탑다운 시점입니다. 좌클릭 또는 터치 드래그로 지도를 이동합니다.</p></article><article class="guide-section"><span class="guide-number">2</span><h3>생산자와 트럭</h3><p>트럭은 생산자 앞까지 빈 차로 이동해 상자 하나를 받은 뒤 하역장으로 돌아옵니다. 생산 시설 15레벨마다 전용 트럭이 1대 추가되며, 배차된 트럭은 사라지지 않습니다.</p></article><article class="guide-section"><span class="guide-number">3</span><h3>하역·선적·정산</h3><p>트럭 한 대는 화물 하나를 하역장에 놓고 크레인이 배에 선적합니다. 현금은 화물을 실은 배가 출항할 때만 들어오며 빈 배의 수익은 0입니다.</p></article><article class="guide-section"><span class="guide-number">4</span><h3>트럭 외형 10단계</h3><p>모든 생산 건물은 전용 트럭 외형을 10단계까지 올릴 수 있습니다. 15레벨마다 해당 건물의 트럭만 새 외형으로 바뀌고 속도가 25% 증가하며, 같은 외형 단계의 트럭은 같은 속도로 달립니다.</p></article><article class="guide-section"><span class="guide-number">5</span><h3>섬 레벨과 새 운송수단</h3><p>레벨 6에는 왼쪽 다리와 Coffee Key가 열립니다. 레벨 8에는 공항이 개방되고 탑다운 비행기가 착륙과 이륙을 반복합니다.</p></article>`)}
