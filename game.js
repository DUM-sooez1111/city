(() => {
  ...
  const WORLD = { minX:-600, w:1880, h:720 };
  const sites = [
  ...
  function resize(){
    ...
    scale=Math.max(r.width/WORLD.w,r.height/(r.width<r.height?620:WORLD.h)); const cameraX=r.width<r.height&&state.level<3?350:WORLD.minX+WORLD.w/2; offsetX=r.width/2-cameraX*scale; offsetY=r.height/2-(WORLD.h/2)*scale;clampCamera();
  ...
  function drawWater(){ctx.fillStyle='#46c6ef';ctx.fillRect(WORLD.minX,0,WORLD.w,WORLD.h);ctx.globalAlpha=.16;ctx.strokeStyle='white';ctx.lineWidth=3;for(let y=35;y<WORLD.h;y+=42){ctx.beginPath();for(let x=WORLD.minX-40;x<WORLD.minX+WORLD.w+40;x+=60){ctx.moveTo(x+(y%80),y);ctx.quadraticCurveTo(x+15+(y%80),y-5,x+31+(y%80),y)}ctx.stroke()}ctx.globalAlpha=1}
  function drawIsland(){
    path(()=>{ctx.moveTo(-570,185);ctx.bezierCurveTo(-530,90,-350,65,-210,135);ctx.bezierCurveTo(-105,190,-95,305,-120,390);ctx.bezierCurveTo(-90,500,-160,625,-300,665);ctx.bezierCurveTo(-455,690,-565,610,-575,480);ctx.closePath()},'#9fca68','#f1dab4',17);
    [[-500,230],[-450,155],[-365,130],[-270,150],[-190,215],[-500,520],[-430,590],[-250,585],[-170,500]].forEach((p,i)=>tree(p[0],p[1],1.15,i%2?'#4d8e35':'#659f3c'));
  ...
  function siteMarkerPosition(s){const positions={banana:[520,390],lumber:[820,555],mine:[1048,225],port:[120,400],coffee:[-440,455]},p=positions[s.id]||[s.x,s.y];return{x:p[0],y:p[1]}}
  ...
  function drawSite(s){
    ...
    if(s.built){
      ...
      if(s.id!=='port'){const producers=sites.filter(p=>p.built&&p.id!=='port'),index=producers.indexOf(s),waiting=Math.max(0,Math.ceil(((state.cargoQueue||0)-index)/Math.max(1,producers.length))),colors={banana:'#e4c738',lumber:'#9f642d',mine:'#8b9598',coffee:'#a96b3f'};for(let i=0;i<Math.min(3,waiting);i++)drawContainer(s.x-17+i*17,s.y+43,colors[s.id]||'#d88b2d')}
    } else {
  ...
  function drawVehicles(){
    ...
    for(let i=0;i<count;i++){
      ...
      const cargoColors={banana:'#e4c738',lumber:'#9f642d',mine:'#8b9598',coffee:'#a96b3f'};
  ...
  function drawNewIslandFog(){if(state.level>=6)return;ctx.save();ctx.beginPath();ctx.moveTo(-590,165);ctx.bezierCurveTo(-540,70,-350,45,-190,120);ctx.bezierCurveTo(-75,180,-75,315,-105,395);ctx.bezierCurveTo(-70,520,-150,660,-295,690);ctx.bezierCurveTo(-475,710,-590,620,-600,470);ctx.closePath();ctx.clip();ctx.fillStyle='rgba(20,39,36,.78)';ctx.fillRect(-600,40,540,670);ctx.globalAlpha=.22;for(let i=0;i<8;i++){ctx.fillStyle='#071d20';ctx.beginPath();ctx.arc(-535+i*62,190+(i%3)*120,55+(i%2)*18,0,7);ctx.fill()}ctx.globalAlpha=1;ctx.fillStyle='white';ctx.textAlign='center';ctx.font='900 38px Nunito';ctx.fillText('🔒',-330,315);ctx.font='900 18px Nunito';ctx.fillText('LEVEL 6',-330,350);ctx.font='900 12px Nunito';ctx.fillText('새로운 섬 · 커피 농장',-330,372);ctx.restore()}
  ...
  function clampCamera(){const r=canvas.getBoundingClientRect(),minOffsetX=r.width-(WORLD.minX+WORLD.w)*scale,maxOffsetX=-WORLD.minX*scale,worldH=WORLD.h*scale;offsetX=Math.min(maxOffsetX,Math.max(minOffsetX,offsetX));offsetY=Math.min(0,Math.max(r.height-worldH,offsetY))}
  ...
  function unlockLevel(s){return({banana:1,lumber:2,mine:3,port:5,coffee:6}[s.id]||1)}
  ...
  function openMap(){openSheet('새로운 섬','ISLAND EXPLORATION',`<div class="wide-card"><span>🏝️</span><div><h3>Emerald Bay</h3><p>현재 개발 중인 무역의 섬</p></div><b>NOW</b></div><div class="wide-card" style="${state.level<6?'filter:grayscale(1);opacity:.65':''}"><span>☕</span><div><h3>Coffee Key</h3><p>${state.level<6?'섬 레벨 6에 왼쪽 다리와 커피 농장 개방':'다리를 건너 커피 농장을 건설할 수 있습니다'}</p></div><b>${state.level<6?'🔒':'OPEN'}</b></div><div class="wide-card" style="${state.level<8?'filter:grayscale(1);opacity:.65':''}"><span>✈️</span><div><h3>Island Airport</h3><p>${state.level<8?'섬 레벨 8에 비행기 운항 개방':'비행기가 착륙·활주·이륙을 반복합니다'}</p></div><b>${state.level<8?'🔒':'OPEN'}</b></div>`)}
  function openGuide(){openSheet('가이드북','MAYOR HANDBOOK',`<article class="guide-section"><span class="guide-number">1</span><h3>작은 섬과 지도</h3><p>모든 시설과 운송수단은 위에서 내려다보는 탑다운 시점입니다. 좌클릭 또는 터치 드래그로 지도를 이동합니다.</p></article><article class="guide-section"><span class="guide-number">2</span><h3>생산자와 트럭</h3><p>트럭은 생산자 앞까지 빈 차로 이동해 상자 하나를 받은 뒤 하역장으로 돌아옵니다. 생산 시설 15레벨마다 전용 트럭이 1대 추가되며, 배차된 트럭은 사라지지 않습니다.</p></article><article class="guide-section"><span class="guide-number">3</span><h3>하역·선적·정산</h3><p>트럭 한 대는 화물 하나를 하역장에 놓고 크레인이 배에 선적합니다. 현금은 화물을 실은 배가 출항할 때만 들어오며 빈 배의 수익은 0입니다.</p></article><article class="guide-section"><span class="guide-number">4</span><h3>운송 업그레이드</h3><p>시설 15레벨마다 해당 생산자의 트럭만 새 외형으로 바뀌고 차량 속도가 25% 증가합니다. 크레인 속도 업그레이드마다 선적 시간이 15% 줄어듭니다.</p></article><article class="guide-section"><span class="guide-number">5</span><h3>섬 레벨과 새 운송수단</h3><p>레벨 6에는 왼쪽 다리와 Coffee Key가 열립니다. 레벨 8에는 공항이 개방되고 탑다운 비행기가 착륙과 이륙을 반복합니다.</p></article>`)}
