(() => {
  ...
  function recordEarnings(amount){state.totalEarned+=amount;state.levelEarnings=(state.levelEarnings||0)+amount;let leveled=false;while(state.levelEarnings>=levelRequirement(state.level)){state.levelEarnings-=levelRequirement(state.level);state.level++;leveled=true}if(leveled)toast(state.level>=6&&state.level-1<6?'섬 레벨 6 달성! 왼쪽 다리와 커피 농장 해금!':`섬 레벨 ${state.level} 달성! 다음 목표 ×3`)}
  ...
  function openMap(){openSheet('새로운 섬','ISLAND EXPLORATION',`<div class="wide-card"><span>🏝️</span><div><h3>Emerald Bay</h3><p>현재 개발 중인 무역의 섬</p></div><b>NOW</b></div><div class="wide-card" style="${state.level<6?'filter:grayscale(1);opacity:.65':''}"><span>☕</span><div><h3>Coffee Key</h3><p>${state.level<6?'섬 레벨 6에 왼쪽 다리와 커피 농장 개방':'다리를 건너 커피 농장을 건설할 수 있습니다'}</p></div><b>${state.level<6?'🔒':'OPEN'}</b></div><div class="wide-card" style="filter:grayscale(1);opacity:.65"><span>🌋</span><div><h3>Volcano Port</h3><p>섬 레벨 8에 개방</p></div><b>🔒</b></div>`)}
  function openGuide(){openSheet('가이드북','MAYOR HANDBOOK',`<article class="guide-section"><span class="guide-number">1</span><h3>작은 섬과 지도</h3><p>모든 시설과 운송수단은 위에서 내려다보는 탑다운 시점입니다. 좌클릭 또는 터치 드래그로 지도를 이동합니다.</p></article><article class="guide-section"><span class="guide-number">2</span><h3>생산자와 트럭</h3><p>트럭은 생산자 앞까지 빈 차로 이동해 상자 하나를 받은 뒤 하역장으로 돌아옵니다. 생산 시설 15레벨마다 전용 트럭이 1대 추가되며, 배차된 트럭은 사라지지 않습니다.</p></article><article class="guide-section"><span class="guide-number">3</span><h3>하역·선적·정산</h3><p>트럭 한 대는 화물 하나를 하역장에 놓고 크레인이 배에 선적합니다. 현금은 화물을 실은 배가 출항할 때만 들어오며 빈 배의 수익은 0입니다.</p></article><article class="guide-section"><span class="guide-number">4</span><h3>운송 업그레이드</h3><p>시설 15레벨마다 해당 생산자의 트럭만 새 외형으로 바뀌고 차량 속도가 25% 증가합니다. 크레인 속도 업그레이드마다 선적 시간이 15% 줄어듭니다.</p></article><article class="guide-section"><span class="guide-number">5</span><h3>섬 레벨과 새 섬</h3><p>선박 출항 수익으로 섬 레벨을 올립니다. 레벨 6에는 왼쪽 다리와 Coffee Key가 개방되고 새로운 커피 생산자를 건설할 수 있습니다.</p></article>`)}
